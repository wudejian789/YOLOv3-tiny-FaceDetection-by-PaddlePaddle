...
class WIDER:
    def __init__(self, imgPath, imgBboxGt, maxBoxNum=64):
        with open(imgBboxGt) as f:
            ...
            bboxArray = np.array(re.findall(r'(\d+) (\d+) (\d+) (\d+).*?\n',txt),dtype='float')
        numArray,bboxArray = self._expansion_rows_to_maxBoxNum(bboxArray,numArray,maxBoxNum)
    ...
    def random_batch_img_stream(self, batchSize=8):
        ...
        while True:
            samples = random.sample(idList,batchSize) if batchSize>0 else random.sample(idList,len(idList))
    ...
    def _expansion_rows_to_maxBoxNum(self, bboxArray, numArray, maxBoxNum):
        ...
        for i in range(numArray.shape[0]):
            if maxBoxNum>numArray[i]:
                obj += numArray[i]
                bboxArray = np.insert(bboxArray,obj=obj,values=np.array([[0,0,0,0] for j in range(maxBoxNum-numArray[i])]),axis=0)
                obj += maxBoxNum-numArray[i]
            else:
                obj += maxBoxNum
                bboxArray = np.delete(bboxArray,obj=[j for j in range(obj,obj+numArray[i]-maxBoxNum)],axis=0)
    ...
    def _scale_img_bbox_array(self, path, imgPathArray, bboxArray, IMG_WH=416):
        ...
        for i in range(imgPathArray.shape[0]):
            ...
            if w>h:
                ...
                dw = (h-w)//2
                dw_e = h-w-dw-dw
            img = cv2.copyMakeBorder(img,dh,dh+dh_e,dw,dw+dw_e,cv2.BORDER_CONSTANT,value=[0,0,0])
            ...
            bboxArray[i,:,0] = (bboxArray[i,:,0]+dw)/(w+dw+dw+dw_e) + bboxArray[i,:,2]/2
            bboxArray[i,:,1] = (bboxArray[i,:,1]+dh)/(h+dh+dh+dh_e) + bboxArray[i,:,3]/2
            
        return np.array(imgArray,dtype='float32').swapaxes(1,3).swapaxes(2,3),np.array(bboxArray,dtype='float32')

def draw_bbox(img, bboxes, show=True, color=(0,0,255), lineWidth=1, savePath=None):
    if type(img)==str:
        img = cv2.imread(img)
    else:
        if img.shape[0] < img.shape[1] and img.shape[0] < img.shape[2]:img = img.swapaxes(0,2).swapaxes(0,1)
    if np.max(img)<=1:
        img *= 255
    img = img.astype('uint8')
    for bbox in bboxes:
        x1,y1,x2,y2 = bbox[:4]
        img = cv2.rectangle(img, ( int(round(x1)),int(round(y1)) ), ( int(round(x2)),int(round(y2)) ), color, lineWidth)
